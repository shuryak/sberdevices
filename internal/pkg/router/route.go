...
func OPTIONS[T Context](pattern string, handlers ...HandlerFunc[T]) *Handler[T] {
	return &Handler[T]{
		regFn: func(r *Router, opts *HandlerOptions[T]) {
			handle(r, "OPTIONS", opts.patternPrefix+pattern, handlers...)
		},
	}
}
...
func SGET[T Context](pattern string, handlers ...HandlerFunc[T]) *Handler[T] {
	return &Handler[T]{
		regFn: func(r *Router, opts *HandlerOptions[T]) {
			handle(r, "GET", opts.patternPrefix+pattern, handlers...)
		},
	}
}

func handle[T Context](r *Router, method, pattern string, handlers ...HandlerFunc[T]) {
	...
	r.mux.HandleFunc(patternWithMethod, func(w http.ResponseWriter, req *http.Request) {
		...
		defer func() {
			if err := recover(); err != nil {
				...
				_ = ctx.WriteResponse(nil, http.StatusInternalServerError) // TODO: error json object
			}
		}()
...
func handleWithDecode[T Context, S Validator[T], U interface{}](
	...
	r.mux.HandleFunc(patternWithMethod, func(w http.ResponseWriter, req *http.Request) {
		...
		ctx.SetHTTPWriter(w)
		ctx.SetHTTPRequest(req)
		ctx.SetErrHandler(func(err error) (interface{}, int) {
			return opts.errHandler(ctx, err)
		})
		...
		if err != nil {
			if opts.errHandler != nil {
				...
				case <-ctx.Done():
					return
				default:
					err = ctx.WriteResponse(data, statusCode)
					if err != nil {
		...
		for _, h := range handlers {
			...
			default:
				...
				if resp != (*U)(nil) {
					err = ctx.WriteResponse(resp, statusCode)
					if err != nil {
